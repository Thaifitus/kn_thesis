...
class BertIntermediate(nn.Module): #where they consider the knowledge neuron
    def __init__(self, config):
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, tgt_pos=None, tgt_layer=None, tmp_score=None, imp_pos=None, imp_op=None):
        ...
        # print(f"sequence_output shape (from custom_bert.py): {sequence_output.size()}") # torch.Size([1, 128, 768]) # torch.Size([20, 128, 768])
...
class BertForMaskedLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, tgt_pos=None, tgt_layer=None, tmp_score=None, tgt_label=None, imp_pos=None, imp_op=None):
        if tmp_score is not None:
            ...
            input_ids = input_ids.repeat(batch_size, 1) # 20, 128
            token_type_ids = token_type_ids.repeat(batch_size, 1) # 20, 128
            attention_mask = attention_mask.repeat(batch_size, 1) # 20, 128
        if imp_op == 'return':
            ...
            last_hidden, ffn_weights = self.bert(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, tgt_pos=tgt_pos, tgt_layer=tgt_layer, tmp_score=tmp_score, imp_pos=imp_pos, imp_op=imp_op)  # (batch, max_len, hidden_size), (batch, max_len, ffn_size) - max_len=128 (token) # ffn_weights==tmp_score when applying ig method (BertIntermediate instance assigns value)
        
        # take result of [MASK] token
        last_hidden = last_hidden[:, tgt_pos, :]  # (batch, hidden_size) # batch=20 (or batch=1)
        ffn_weights = ffn_weights[:, tgt_pos, :]  # (batch, ffn_size)
        # preidct masked token
        ...
        if imp_op == 'return':
            ...
            if tmp_score is None:
                ...
                '''
                -> torch downgrades tensor dimension after splitting.
                tgt_prob.size() == torch.Size([20, 28996])
                tgt_prob[:, tgt_label].size() == torch.Size([20])
                torch.unbind(..).size() == 20 e.g. (tensor(0.4381), tensor(0.2008),...)
                '''
